...
2. OpenAI API 과금 관련하여서는 만약 API 과금이 되었을 때는 그게 페이가 않 되었을 시 응답 대신 과금에러가 뜬다고 합니다. 결제에서 API 사용요금 선충전/후사용하라고 합니다만 저는 아직 충전은 않했고, 현재 까지 매우 간단한 메시지 몇 개만(아마 전체 ~100토큰 미만)을 보내서 성공 여부를 테스트해 본 거라서 그랬는지 아직까지 API 과금은 없었습니다. 테스트 해보실 때 저처럼 간단한 메시지부터 보내 성공여부를 테스트해 보시고 이 후에 일정 금액을 충전하고 사용하시면 될 것 같습니다.
