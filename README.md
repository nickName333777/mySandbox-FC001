## [bite-size 기능 구현] Spring AI를 이용한 챗봇 구현 by 초급 자바 개발자
<br>
<br>
...
2. OpenAI API 과금 관련하여서는 만약 API 사용 요금이 부과 되었을 때 그걸 페이 않하면 응답 대신 과금 에러가 뜬다고 합니다. 결제에서 API 사용 요금을 선충전/후사용 하라고 합니다만 저는 아직 충전은 않했고, 현재 까지 매우 간단한 메시지 몇 개만(아마 전체 ~100토큰 미만)을 보내서 성공 여부를 테스트해 본 거라서 그랬는지 아직까지 API 과금은 없었습니다. 테스트 해보실 때 저처럼 간단한 메시지부터 보내 성공여부를 테스트해 보시고 이 후에 일정 금액을 충전하고 사용하시면 될 것 같습니다.
3. 관련하여 OpenAI API과금이 토큰 수 기준이라 토큰 수 정보가 있으면 참고가 될 것 같아 토큰 수에 대한 서버측 계산과 클라이이언트측 추정값을 보고 싶어 해당내용을 화면에 포함해 보았는데, 서버측 계산 토큰값은 메타정보를 가져오는데 어려움이 있어 그냥 더미값을 넣어 준거라 의미가 없는 값이고(추후 수정예정), 클라이언트 추정 토큰은 4글자당 1 토큰으로 대략 계산한 값 이므로 API사용량 별 과금액수를 추정 하실 때 대략적인 참고 값으로만 봐주시면 좋을 것 같습니다.(좀더 정확한 토큰 값을 원하시는 분은 JS 계산부분에 토큰나이저를 붙여 보세요. Update: 서버측 계산 토큰값 메타정보를 가져와 토큰사용에 따른 과금을 부과하는 유료 챗봇을 파이널 프로젝트에서 구현해 보았으니 파이널 프로젝트 repo에서 해당 내용을 참조해 주세요: https://github.com/nickName333777/DevlogProject, 또는 https://github.com/Team-ZeroBoost/Devlog )
