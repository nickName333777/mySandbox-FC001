...
### 기타:
1. 원래 국내 LLM을 달아보고 싶었는데 Naver-HyperClovX와  LG-Exaone은 Spring AI에서는 지원이 않되서 OpenAI-gpt-4o-mini(가성비?)로 하게 되었음.
2. OpenAI API 과금 관련하여, 현재 까지 매우 간단한 메시지 몇 개만(아마 전체 ~100토큰 미만)을 성공 여부 테스트로 보낸거라서 그랬는지 API 과금은 아직까지 없음. 테스트시 간단한 메시지 부터 시작해 보시고, 만약 API 과금이 되었을 때 그게 페이가 않되면 과금에러가 뜬다고 함.  
3. OpenAI API과금이 토큰수 기준이라 토큰 수 정보가 있으면 참고가 될 것 같아 서버측&클라이이언트측 토큰수 추정값을 보고 싶어 포함해 보았는데, 서버계산 토큰값은 메타정보를 가져오는데 어려움이 있어 그냥 더미값을 넣어줘 의미없는 값임. 클라이언트 추정 토큰은 4글자당 1 토큰으로 계산한 대략 값이므로 API사용량 별 과금액수 추정에  대략적인  참고로만 볼것.(좀더 정확한 토큰 값을 원하시면 JS 계산부분에 토큰나이저를 붙여 보세요)
